...
const LEGACY_GOTHAM_RESIDENT_ID_MAP = Object.freeze({
    '1': 'gotham-bruce',
    '2': 'gotham-dick',
    '3': 'gotham-jason',
    '4': 'gotham-tim',
...
const canonicalizeLegacyGothamResidentId = (value) => {
    const normalized = String(value ?? '').trim();
    return LEGACY_GOTHAM_RESIDENT_ID_MAP[normalized] || value;
};
const DEFAULT_HALLS = [
...
const normalizeCatHall = (cat) => {
    ...
    const normalized = {
        ...
        // A persisted numeric value—including the former default of 10—is
        // authoritative. Only genuinely absent or invalid values are repaired.
        affinity: (cat.affinity !== null && cat.affinity !== undefined && cat.affinity !== '' && Number.isFinite(Number(cat.affinity)))
            ? Number(cat.affinity)
...
const mergeObsoleteIthacaCat = (legacyCat, canonicalCat, canonicalProfile, targetId) => {
    ...
    // This merge reconciles duplicate roster identities, not relationship state.
    // Keep the most recently lived persisted value (including exactly 10) rather
    // than allowing a fresh template baseline to overwrite it.
    const latestAffinity = latest?.affinity;
    const fallbackAffinity = latest === legacyCat ? canonicalCat?.affinity : legacyCat?.affinity;
    merged.affinity = (latestAffinity !== null && latestAffinity !== undefined && latestAffinity !== '' && Number.isFinite(Number(latestAffinity)))
        ? Number(latestAffinity)
        : ((fallbackAffinity !== null && fallbackAffinity !== undefined && fallbackAffinity !== '' && Number.isFinite(Number(fallbackAffinity)))
            ? Number(fallbackAffinity)
            : DEFAULT_RESIDENT_AFFINITY);
    ...
    Object.assign(data, {
        DEFAULT_RESIDENT_AFFINITY,
        defaultCats,
        LEGACY_GOTHAM_RESIDENT_ID_MAP,
        canonicalizeLegacyGothamResidentId,
        DEFAULT_HALLS,
        makeHallCat,
        defaultMarvelCats,
        defaultGothamExpansionCats,
